...
const Brands = () => {
    ...
    const getAllBrand = async () => {
        try {
            ...
            console.log(err);
            setLoading(false); // Set loading to false even on error to stop spinner
        }
    };
                ...
                <div className="container">
                    ...
                    {loading ?
                        ...
                        <div className="row justify-content-center">
                            {brands?.map((c, index) => (
                                <div key={c._id || index} className="col-lg-3 col-md-4 col-sm-6 mb-4 showcase_card">
                                    <Link to={`/brand/${c.slug}`}>
                                        <img
                                            ...
                                            title={c.name}
                                            alt={c.name} // Added alt attribute for accessibility
                                        />
                                    </Link>
                                </div>
                            ))}
