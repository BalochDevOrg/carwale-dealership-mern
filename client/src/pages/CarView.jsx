...
import { getBrandImage, getCarImage, getImageUrl } from '../utils/getImageUrl';
...
const CarView = () => {
    ...
    const getCar = async () => {
        try {
            const { data } = await axios.get(`${process.env.REACT_APP_API_URL}/api/car/get-car/${params.slug}`);
            setCar(data.car);
    ...
    const galleryImages = (car.productPictures || []).map((picture) => ({
        original: getImageUrl(picture),
        thumbnail: getImageUrl(picture),
    }));
    const turntableImages = (car.productPictures || []).map((picture) => getImageUrl(picture));
            ...
            {loading ?
                ...
                <div className="row">
                    <div className="col-md-6 text-center">
                        <ReactImageTurntable
                            images={turntableImages.length ? turntableImages : [getCarImage([])]}
                    ...
                    <div className="col-md-6">
                        <div className='centerMob'>
                            <Link to={`/brand/${car.brand.name}`}>
                                <img
                                    decoding="async"
                                    src={getBrandImage(car.brand.brandPictures)}
                                    className="img-fluid"
                                    ...
                                    <div className="col-md-12 col-lg-3 mb-3 mb-lg-0 my-3">
                                        <div className="card">
                                            ...
                                            <Link to={`/car/${p.slug}`} className='text-center'>
                                                <img src={getCarImage(p.productPictures)} alt={p.name} style={{ maxWidth: '100%', maxHeight: '120px', objectFit: 'contain' }} className='border rounded' />
                                            </Link>
